...
const Layout = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        const response = await fetch("https://server-three-gamma-87.vercel.app/api/v1/user/details", {
          method: "GET",
          headers: {
  ...
  const fetchBalance = useCallback(async () => {
    try {
      const response = await fetch("https://server-three-gamma-87.vercel.app/pi/v1/account/balance", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `${localStorage.getItem("token")}`,
